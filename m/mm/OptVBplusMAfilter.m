...
folder = 'D:\future\VBplusMAfilter 优化记录\';
%% 分析 TB 参数报告
...
if exist(filename,'file')
    ...
    figure; 
    subplot(2,1,1); surf(param1,param2,profit);view(2); axis tight; colorbar; title('橡胶 ru888'); 
...
if exist(filename,'file')
    ...
    figure; 
    subplot(2,1,1); surf(param1,param2,profit);view(2); axis tight; colorbar; title('股指 IF888');  
    map = mycolormap(profit); colormap(map); 
...
if exist(filename,'file')
    ...
    figure; 
    subplot(2,1,1); surf(param1,param2,profit);view(2); axis tight; colorbar; title('沪锌 zn888'); 
    map = mycolormap(profit); colormap(map); 
...
filename = [folder '优化记录[v9888 - 1日线].csv'];
if exist(filename,'file')
    dstruct = importdata(filename);
    display(dstruct.textdata(1,:));
    data = dstruct.data;
