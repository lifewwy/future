function [varargout] = spiderplot(X, axisMax, axisMin, colors,indicator)
...
for i=1:(length(x)/3),
    if indicator{i}(end)=='%'
        indicator(i) = {indicator{i}(1:end-1)};
    end
    %     labels(i) = text((x(i*3-1)-1)*1.1+1, (y(i*3-1)-1)*1.1+1, ...
    %         sprintf(['%g ',indicator{i}, ' %g'],axisMin(i), axisMax(i)));
    labels(i) = text((x(i*3-1)-1)*1.1+1, (y(i*3-1)-1)*1.1+1, ...
        sprintf([indicator{i}, ' %g'],xx(i) ));
