function [OpenDate,CloseDate,parameter,ContractName] = TradingPosTB01(ffilename,SelectedContract)
% clear; close all; clc;

if nargin == 0
    directory = 'C:\D\future\交易位置\';
    SelectedContract = 'PTA';
    % filename = ['20170918交易位置.txt'];
    % yyyymmdd = datestr(now,'yyyymmdd');
    ffilename = [directory,'交易位置20170918','.txt'];
end
...
for k = 1:length(tradingAction)
    ...
    if strcmp(tradingAction{k},'|')
        ...
        nPeriodBreak = str2double(d{k,8});
        nATR = d1.data(1);
        parameter = [nATRExit,nPeriodBreak,nATR];
    end
end  
